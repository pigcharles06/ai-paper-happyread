...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const uploadInput = document.getElementById('pdf-upload'); // Correct ID
    ...
    const pageNumInput = document.getElementById('page-num-input'); // Correct ID
    ...
    const clearDataBtn = document.getElementById('clear-data-btn'); // Clear button reference
    ...
    // Use the correct IDs for verification
    const essentialIds = [
        'pdf-viewer-container', 'text-layer', 'pdf-loader', 'pdf-upload', // Use correct ID
        'upload-status', 'paper-select', 'chat-messages', 'chat-input',
        'send-chat-btn', 'prev-page', 'next-page', 'page-num-input', // Use correct ID
        'page-count', 'selected-text-display', 'translate-btn',
        'instant-translate-toggle', 'translation-tooltip', 'analyze-page-btn', 'voice-toggle',
        'clear-data-btn' // Add clear button ID
    ];
    ...
    if(missingElement) {
        alert("頁面初始化錯誤，部分介面元素遺失，功能可能不完整。");
         ...
         console.log("All essential DOM elements verified (initial check).");
    }
    ...
    function initializeCanvas() {
        ...
        pdfCanvas.style.display = 'block'; pdfCanvas.style.margin = '0 auto';
        pdfCanvas.style.pointerEvents = 'none';
        pdfViewerContainer.insertBefore(pdfCanvas, textLayerDiv);
    ...
    async function renderPage(num) {
        ...
        try {
            ...
            textLayerDiv.style.width = `${viewport.width}px`; textLayerDiv.style.height = `${viewport.height}px`; textLayerDiv.style.position = 'absolute'; textLayerDiv.style.top = '0'; textLayerDiv.style.left = '0';
    ...
    function addChatMessage(sender, message, type = 'normal', clearPrevious = false) {
        ...
        if(clearPrevious) {
             chatMessages.innerHTML = '<div class="initial-chat-prompt text-gray-500 dark:text-gray-400 italic text-sm p-2 text-center">請先上傳或選擇一篇論文...</div>'; // Reset to initial prompt
        }
    ...
    async function sendChatMessage() {
        ...
        const payload = { message: message, paper_id: selectedPaperId || null, currentPageNum: currentPageNum }; // Pass currentPageNum
        ...
        try {
            ...
            else { console.log("Voice mode off or playVoiceResponse fn unavailable."); }
        } catch (error) { console.error('Chat error:', error); addChatMessage('system', `請求錯誤: ${error.message || error}`, 'error');
        } finally { chatInput.disabled = false; sendChatBtn.disabled = false; sendChatBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>`; chatInput.focus(); }
    }
    ...
    if(voiceToggle) voiceToggle.addEventListener('change', () => { /* Logic handled in voice.js */ if (voiceToggle.checked) { console.log("Voice toggle checked (main.js)"); } else { console.log("Voice toggle unchecked (main.js)"); } });
    if(clearDataBtn) clearDataBtn.addEventListener('click', async () => { const confirmation = window.confirm("危險操作！\n\n您確定要清除所有已上傳的論文 PDF 檔案以及相關的 RAG 分析資料嗎？\n\n這個操作無法復原！"); if (confirmation) { console.log("User confirmed data clearing."); setUploadStatus("正在清除資料...", "loading"); clearDataBtn.disabled = true; const originalBtnHtml = clearDataBtn.innerHTML; clearDataBtn.innerHTML = `<svg class="animate-spin h-4 w-4 inline mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> 清除中...`; try { const response = await fetch('/clear_data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, }); const result = await response.json(); if (response.ok) { console.log("Data cleared on backend."); setUploadStatus(result.message || "資料已成功清除。", "success"); clearPdfDisplay(); if(paperSelect) paperSelect.options.length = 1; if(chatMessages) { chatMessages.innerHTML = '<div class="initial-chat-prompt text-gray-500 dark:text-gray-400 italic text-sm p-2 text-center">請先上傳或選擇一篇論文...</div>'; } if(selectedTextDisplay) selectedTextDisplay.textContent = '請在 PDF 中選取文字...'; selectedText = ''; if(translateBtn) translateBtn.disabled = true; if(instantTranslateToggle) instantTranslateToggle.checked = false; hideTranslationTooltip(); if(analyzePageBtn) analyzePageBtn.disabled = true; } else { console.error("Error clearing data:", result.error, result.details); setUploadStatus(`清除資料失敗: ${result.error || '未知錯誤'}`, "error"); } } catch (error) { console.error("Clear data request error:", error); setUploadStatus(`清除請求失敗: ${error.message || '網路錯誤'}`, "error"); } finally { clearDataBtn.disabled = false; clearDataBtn.innerHTML = originalBtnHtml; } } else { console.log("User cancelled data clearing."); } });
    ...
    // Assign AFTER they are defined within this scope.
    window.addChatMessage = addChatMessage;
    window.sendChatMessage = sendChatMessage;
    // Note: playVoiceResponse is attached to window in voice.js
